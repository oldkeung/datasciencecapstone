...
#           L-M-N     5 times
# ==> we have N5 = 4
createThreeGramTableExtended = function(threeGramTable){
  ...
  for(i in 5:1){
    ...
    currN = nrow(threeGramTable[freq == currRTimes])
    nextN = nrow(threeGramTable[freq == nextRTimes])
    
    currd = (nextRTimes / currRTimes) * (nextN / currN) # assumption: 0 < d < 1
    
    # the beauty of "data.table"!
    threeGramTable[freq == currRTimes, discount := currd]
  }
  
  # Calculate the remaining probability (thanks to discounting...).
  threeGramTable_leftOverProb = threeGramTable[, .(leftoverprob=calcLeftOverProb(predict, freq, discount)), by=base]
