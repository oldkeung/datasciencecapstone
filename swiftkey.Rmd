...
Due to the popularity of mobile devices in recent years, people has performed many daily activities, like email or social networking, in their mobile devices. There is increasing demand to improve the typing experience on the devices. 
...
In this report, we will work on 
...
#badWord
#nfeat(alldfm)
#nfeat(dfm_select(alldfm, "shxt", selection = 'remove'))
```
...
printWordCloud <- function(dfm, maxWordCount)
{
  ...
  wordcloud(words = names(topFeatures), freq = topFeatures, max.words = maxWordCount, random.order = FALSE, rot.per = 0.35, colors = brewer.pal(8, "Dark2"))
}
```
